...
def main():
    ...
    pool.append(config( \
        equip=['t20','ring','wrist'],talent=['black','ht15']   \
        ))
    pool.append(config(\
        equip=['t20','ring','waist'],talent=['black','ht15']   \
        ))
...
def test2(pool, time = 100000):
    if len(pool) != 2:
        print ' pool != 2 '
        exit()
    a = brm(conf = pool[0], t20rppm = 5, newfuzan = 1)
    b = brm(conf = pool[1], t20rppm = 5, newfuzan = 1)
    a.run(time)
    b.run(time)
    ar = a.showavoid()
    print '>'
